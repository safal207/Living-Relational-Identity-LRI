...
def test_continuity_hashing():
    ...
    # 2. Evolve (First Step)
    decision1 = {"action": "step1", "intention": "init", "timestamp": "2023-01-01T10:00:00Z"}
    ...
    decision2 = {"action": "step2", "intention": "follow_up", "timestamp": "2023-01-01T10:01:00Z"}
...
def test_authority_policy_drift():
    ...
    assert policy.is_authorized(identity, "normal_action", {}, drift_score=0.1)
    assert policy.is_authorized(identity, "critical_action", {}, drift_score=0.1)

    # Case 2: High drift, critical action rejected
    assert policy.is_authorized(identity, "normal_action", {}, drift_score=0.9)
    assert not policy.is_authorized(identity, "critical_action", {}, drift_score=0.9)
